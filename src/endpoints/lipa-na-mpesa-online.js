 ...
 * @return {Promise}
 */
module.exports = async function (senderMsisdn, amount, callbackUrl, accountRef, transactionDesc = 'Lipa na mpesa online', transactionType = 'CustomerPayBillOnline', shortCode = null, passKey = null, businessShortCode = null) {
  const _businesShortCode = (businessShortCode || shortCode) || this.configs.lipaNaMpesaShortCode
  ...
  const password = Buffer.from(`${_businessShortCode}${_passKey}${timeStamp}`).toString('base64')
  const req = await this.request()
  return req.post('/mpesa/stkpush/v1/processrequest', {
    'BusinessShortCode': _businesShortCode,
    'Password': password,
    'Timestamp': timeStamp,
